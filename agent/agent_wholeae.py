...
class WholeAEAgent(BaseAgent):
    def __init__(self, config):
        super(WholeAEAgent, self).__init__(config)
        self.points_batch_size = config.points_batch_size
        self.resolution = config.resolution
        self.batch_size = config.batch_size

    def build_net(self, config):
        net = get_network('whole_ae', config)
        print('-----Whole AE architecture-----')
        print(net)
        if config.parallel:
    ...
    def visualize_batch(self, data, mode, outputs=None):
        ...
        voxel_proj = project_voxel_along_xyz(voxel, concat=True)
